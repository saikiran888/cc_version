...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QSpinBox, QScrollArea, QGroupBox, QDialog, QSplitter, QShortcut, QToolButton
)
...
class VideoLabel(QLabel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Allow Qt layouts to size us; we'll enforce square in heightForWidth()
...
class SingleVideoPlayer(QWidget):
    def __init__(self, parent=None, player_id=1, min_size=300, max_size=500):
    ...
    def setup_ui(self):
        ...
        # Video container - expanded to cover waste space
        video_container = QWidget()
        video_layout = QHBoxLayout(video_container)
        video_layout.setContentsMargins(0, 0, 0, 0)
        video_layout.setSpacing(0)

        # Video display - moved to corner and expanded
        self.video_label = VideoLabel()
        self.video_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        # self.video_label.setMinimumSize(400, 400)
        # self.video_label.setMaximumSize(800, 800)
        ...
        # Combined Playback and Nanowell Controls - all in one line
        controls_group = QGroupBox("Controls")
        controls_layout = QVBoxLayout()
        controls_layout.setSpacing(10)
        
        # Main controls row - all buttons in one line, in the requested order
        ...
        main_controls_layout.addWidget(self.prev_frame_button)
        
        # Previous UID (nanowell) button
        ...
        """)
        main_controls_layout.addWidget(self.prev_uid_button)

        # Play/Pause button
        self.play_pause_button = QPushButton("||")
        self.play_pause_button.setFixedWidth(60)
        self.play_pause_button.clicked.connect(self.toggle_playback)
        main_controls_layout.addWidget(self.play_pause_button)

        # Next UID (nanowell) button
        ...
        """)
        main_controls_layout.addWidget(self.next_uid_button)
        
        # Next frame button
        self.next_frame_button = QPushButton(">>")
        self.next_frame_button.setFixedWidth(60)
        self.next_frame_button.clicked.connect(self.next_frame)
        main_controls_layout.addWidget(self.next_frame_button)
        ...
        controls_layout.addWidget(self.nanowell_info_label)
    ...
    def set_video_data(self, images, boxes, channel_images, uid, run_dirname, continuation=False):
        print(f"[DEBUG] SingleVideoPlayer.set_video_data: {len(images)} images, {len(boxes)} boxes, {len(channel_images)} channel_images for UID {uid}")
    ...
    def switch_images(self, image_index=-1):
        ...
        try:
            ...
            # Create a blank pixmap for the label size
            final = QPixmap(label_size)
            final.fill(QColor('#f0f0f0'))
            ...
            painter.end()
            self.video_label.setPixmap(final)
    ...
    def keyPressEvent(self, event):
        """Handle keyboard events for nanowell navigation"""
        if event.key() == Qt.Key_Left:
            self.on_prev_uid_clicked()
            event.accept()
        elif event.key() == Qt.Key_Right:
            self.on_next_uid_clicked()
            event.accept()
...
class DualVideoWindow(QDialog):
    def __init__(self, parent=None):
        ...
        self.shortcut_fullscreen = QShortcut(Qt.Key_F11, self)
    ...
    def setup_ui(self):
        ...
        # Left side: Videos only (expanded to cover waste space)
        videos_container = QWidget()
        videos_container.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        videos_layout = QVBoxLayout(videos_container)
        videos_layout.setContentsMargins(0, 0, 0, 0)
        videos_layout.setSpacing(10)

        # Top video player - expanded
        top_group = QGroupBox("Video 1")
        top_layout = QVBoxLayout(top_group)
        self.player1 = SingleVideoPlayer(self, player_id=1, min_size=600, max_size=1000)
        self.player1.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        top_layout.addWidget(self.player1)
        videos_layout.addWidget(top_group)

        # Bottom video player - expanded
        bottom_group = QGroupBox("Video 2")
        bottom_layout = QVBoxLayout(bottom_group)
        self.player2 = SingleVideoPlayer(self, player_id=2, min_size=600, max_size=1000)
        self.player2.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        # Fullscreen button at the top right of controls
        self.fullscreen_btn = QToolButton()
        self.fullscreen_btn.setText("⛶ Fullscreen")
        self.fullscreen_btn.setToolTip("Toggle Fullscreen (F11)")
        self.fullscreen_btn.setCheckable(True)
        self.fullscreen_btn.clicked.connect(self.toggle_fullscreen)
        controls_layout.addWidget(self.fullscreen_btn, alignment=Qt.AlignRight)
        ...
        main_layout.addWidget(videos_container, stretch=3)  # 30%
        main_layout.addWidget(controls_container, stretch=7)  # 70%

    def setup_analysis_overlay_tab(self):
        tab = QWidget()
        layout = QVBoxLayout(tab)
        
        # --- Top row: Analysis Parameters and Colors side by side ---
        top_row = QHBoxLayout()
        # Analysis Parameters
        ...
        self.edint_entry.setFixedWidth(100)
        self.edint_entry.setStyleSheet("padding: 6px; font-size: 14px;")
        analysis_layout.addWidget(self.edint_entry, 0, 1)
        analysis_layout.addWidget(QLabel("DInt:"), 1, 0)
        self.dint_entry = QLineEdit("125")
        self.dint_entry.setFixedWidth(100)
        self.dint_entry.setStyleSheet("padding: 6px; font-size: 14px;")
        ...
        top_row.addWidget(analysis_group)
        # Colors group (from setup_colors_channels_tab)
        color_group = QGroupBox("Colors")
        color_layout = QHBoxLayout()
        color_layout.setSpacing(20)
        self.eff_color = QColor('#FFC20A')
        self.tar_color = QColor('#0C7BDC')
        from color_picker import ColorPickerWidget
        self.eff_color_picker = ColorPickerWidget(self.eff_color, "Effector:")
        self.eff_color_picker.setMinimumWidth(120)
        self.eff_color_picker.setMaximumWidth(180)
        self.eff_color_picker.setMinimumHeight(40)
        self.tar_color_picker = ColorPickerWidget(self.tar_color, "Target:")
        self.tar_color_picker.setMinimumWidth(120)
        self.tar_color_picker.setMaximumWidth(180)
        self.tar_color_picker.setMinimumHeight(40)
        color_layout.addWidget(self.eff_color_picker)
        color_layout.addWidget(self.tar_color_picker)
        color_group.setLayout(color_layout)
        top_row.addWidget(color_group)
        layout.addLayout(top_row)

        # --- Video 1 Controls ---
        playback_group1 = QGroupBox("Video 1 Controls")
        playback_layout1 = QVBoxLayout()
        playback_layout1.setSpacing(10)
        self.nanowell_info_label1 = QLabel("Nanowell info: UID: N/A")
        playback_layout1.addWidget(self.nanowell_info_label1)
        main_controls_layout1 = QHBoxLayout()
        main_controls_layout1.setSpacing(10)
        ...
        main_controls_layout1.addWidget(self.prev_frame_button1)
        self.prev_uid_button1 = QPushButton("<")
        self.prev_uid_button1.setFixedWidth(40)
        self.prev_uid_button1.setToolTip("Previous UID")
        self.prev_uid_button1.clicked.connect(lambda: self.previous_uid_for_player(1))
        self.prev_uid_button1.setStyleSheet("""
            QPushButton { background-color: rgba(33, 150, 243, 0.7); color: white; border: none; border-radius: 0; padding: 6px 12px; font-weight: bold; font-size: 16px; }
            QPushButton:hover { background-color: rgba(25, 118, 210, 0.9); }
            QPushButton:pressed { background-color: rgba(13, 71, 161, 1); }
        """)
        main_controls_layout1.addWidget(self.prev_uid_button1)
        ...
        main_controls_layout1.addWidget(self.play_pause_button1)
        self.next_uid_button1 = QPushButton(">")
        self.next_uid_button1.setFixedWidth(40)
        self.next_uid_button1.setToolTip("Next UID")
        self.next_uid_button1.clicked.connect(lambda: self.next_uid_for_player(1))
        self.next_uid_button1.setStyleSheet("""
            QPushButton { background-color: rgba(33, 150, 243, 0.7); color: white; border: none; border-radius: 0; padding: 6px 12px; font-weight: bold; font-size: 16px; }
            QPushButton:hover { background-color: rgba(25, 118, 210, 0.9); }
            QPushButton:pressed { background-color: rgba(13, 71, 161, 1); }
        """)
        main_controls_layout1.addWidget(self.next_uid_button1)
        ...
        main_controls_layout1.addWidget(self.next_frame_button1)
        playback_layout1.addLayout(main_controls_layout1)
        ...
        # --- Video 1 Overlay and Channel Overlay side by side ---
        overlay_row1 = QHBoxLayout()
        # Video 1 Overlay Options
        ...
        self.show_boxes_checkbox1.stateChanged.connect(lambda: self.update_overlay_options(1))
        ...
        self.show_text_checkbox1.stateChanged.connect(lambda: self.update_overlay_options(1))
        video1_layout.addWidget(self.show_text_checkbox1, 0, 1)
        self.show_contours_checkbox1 = QCheckBox("Show Contours")
        self.show_contours_checkbox1.stateChanged.connect(lambda: self.update_overlay_options(1))
        ...
        self.show_base_checkbox1.stateChanged.connect(lambda: self.update_overlay_options(1))
        video1_layout.addWidget(self.show_base_checkbox1, 1, 1)
        video1_group.setLayout(video1_layout)
        overlay_row1.addWidget(video1_group)
        # Video 1 Channel Overlay
        ...
        overlay_row1.addWidget(channel1_group)
        # Add to layout
        layout.addWidget(playback_group1)
        layout.addLayout(overlay_row1)

        # --- Video 2 Controls ---
        playback_group2 = QGroupBox("Video 2 Controls")
        playback_layout2 = QVBoxLayout()
        playback_layout2.setSpacing(10)
        self.nanowell_info_label2 = QLabel("Nanowell info: UID: N/A")
        playback_layout2.addWidget(self.nanowell_info_label2)
        main_controls_layout2 = QHBoxLayout()
        main_controls_layout2.setSpacing(10)
        ...
        main_controls_layout2.addWidget(self.prev_frame_button2)
        self.prev_uid_button2 = QPushButton("<")
        self.prev_uid_button2.setFixedWidth(40)
        self.prev_uid_button2.setToolTip("Previous UID")
        self.prev_uid_button2.clicked.connect(lambda: self.previous_uid_for_player(2))
        self.prev_uid_button2.setStyleSheet("""
            QPushButton { background-color: rgba(33, 150, 243, 0.7); color: white; border: none; border-radius: 0; padding: 6px 12px; font-weight: bold; font-size: 16px; }
            QPushButton:hover { background-color: rgba(25, 118, 210, 0.9); }
            QPushButton:pressed { background-color: rgba(13, 71, 161, 1); }
        """)
        main_controls_layout2.addWidget(self.prev_uid_button2)
        ...
        main_controls_layout2.addWidget(self.play_pause_button2)
        self.next_uid_button2 = QPushButton(">")
        self.next_uid_button2.setFixedWidth(40)
        self.next_uid_button2.setToolTip("Next UID")
        self.next_uid_button2.clicked.connect(lambda: self.next_uid_for_player(2))
        self.next_uid_button2.setStyleSheet("""
            QPushButton { background-color: rgba(33, 150, 243, 0.7); color: white; border: none; border-radius: 0; padding: 6px 12px; font-weight: bold; font-size: 16px; }
            QPushButton:hover { background-color: rgba(25, 118, 210, 0.9); }
            QPushButton:pressed { background-color: rgba(13, 71, 161, 1); }
        """)
        main_controls_layout2.addWidget(self.next_uid_button2)
        ...
        main_controls_layout2.addWidget(self.next_frame_button2)
        playback_layout2.addLayout(main_controls_layout2)
        ...
        # --- Video 2 Overlay and Channel Overlay side by side ---
        overlay_row2 = QHBoxLayout()
        # Video 2 Overlay Options
        ...
        self.show_boxes_checkbox2.stateChanged.connect(lambda: self.update_overlay_options(2))
        ...
        self.show_text_checkbox2.stateChanged.connect(lambda: self.update_overlay_options(2))
        video2_layout.addWidget(self.show_text_checkbox2, 0, 1)
        self.show_contours_checkbox2 = QCheckBox("Show Contours")
        self.show_contours_checkbox2.stateChanged.connect(lambda: self.update_overlay_options(2))
        ...
        self.show_base_checkbox2.stateChanged.connect(lambda: self.update_overlay_options(2))
        video2_layout.addWidget(self.show_base_checkbox2, 1, 1)
        video2_group.setLayout(video2_layout)
        overlay_row2.addWidget(video2_group)
        # Video 2 Channel Overlay
        ...
        overlay_row2.addWidget(channel2_group)
        # Add to layout
        layout.addWidget(playback_group2)
        layout.addLayout(overlay_row2)

        self.control_tabs.addTab(tab, "Analysis & Overlay")

        # --- Real-time update connections ---
        # Update right-panel controls as video plays
        self.player1.timer.timeout.connect(self.update_right_panel_controls)
        self.player2.timer.timeout.connect(self.update_right_panel_controls)
        # Also update nanowell info when UID changes (call in set_video_data)
    ...
    def update_right_panel_controls(self):
        ...
        if hasattr(self.player1, 'images') and self.player1.images:
            ...
            self.nanowell_info_label1.setText(f"Nanowell info: UID: {self.player1.selected_uid}")
        ...
        if hasattr(self.player2, 'images') and self.player2.images:
            ...
            self.nanowell_info_label2.setText(f"Nanowell info: UID: {self.player2.selected_uid}")
    ...
    def setup_colors_channels_tab(self):
        ...
        # Color pickers with required size
        self.eff_color = QColor('#FFC20A')  # Yellow
        self.tar_color = QColor('#0C7BDC')  # Blue
        try:
            from color_picker import ColorPickerWidget
        ...
        channel1_group.setLayout(channel1_layout)
        layout.addWidget(channel1_group)
        
        # Video 2 Channel Overlays (bottom)
        channel2_group = QGroupBox("Video 2 Channel Overlays")
        channel2_layout = QGridLayout()
        self.channel_overlay_checkboxes2 = {}
        self.channel_thresholds2 = {'CH1': 200, 'CH2': 200, 'CH3': 200}
        ...
        channel2_group.setLayout(channel2_layout)
        layout.addWidget(channel2_group)
        
        self.control_tabs.addTab(tab, "Colors_Channels")
    ...
    def setup_plots_metrics_tab(self):
        ...
        # --- Video 1 Plot ---
        plot_container1 = QGroupBox("Video 1 Nanowell Metrics Over Time")
        plot_layout1 = QVBoxLayout(plot_container1)
        from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
        from matplotlib.figure import Figure
        self.figure1 = Figure(figsize=(6, 3), dpi=100)
        self.canvas1 = FigureCanvas(self.figure1)
        plot_layout1.addWidget(self.canvas1)
        # Plot controls for Video 1
        plot_controls1 = QHBoxLayout()
        plot_controls1.addWidget(QLabel("X Axis:"))
        self.x_axis_combo1 = QComboBox()
        self.x_axis_combo1.addItems(["Timepoints"])
        plot_controls1.addWidget(self.x_axis_combo1)
        plot_controls1.addWidget(QLabel("Y Axis:"))
        self.y_axis_combo1 = QComboBox()
        self.y_axis_combo1.addItems([
            "Min", "Max", "Median", "Mean",
            "CH3 Annexin Intensity", "Cell Count", "Survival Curve"
        ])
        self.y_axis_combo1.currentTextChanged.connect(lambda: self.update_plot_metric(1))
        plot_controls1.addWidget(self.y_axis_combo1)
        plot_layout1.addLayout(plot_controls1)
        layout.addWidget(plot_container1)

        # --- Video 2 Plot ---
        plot_container2 = QGroupBox("Video 2 Nanowell Metrics Over Time")
        plot_layout2 = QVBoxLayout(plot_container2)
        self.figure2 = Figure(figsize=(6, 3), dpi=100)
        self.canvas2 = FigureCanvas(self.figure2)
        plot_layout2.addWidget(self.canvas2)
        # Plot controls for Video 2
        plot_controls2 = QHBoxLayout()
        plot_controls2.addWidget(QLabel("X Axis:"))
        self.x_axis_combo2 = QComboBox()
        self.x_axis_combo2.addItems(["Timepoints"])
        plot_controls2.addWidget(self.x_axis_combo2)
        plot_controls2.addWidget(QLabel("Y Axis:"))
        self.y_axis_combo2 = QComboBox()
        self.y_axis_combo2.addItems([
            "Min", "Max", "Median", "Mean",
            "CH3 Annexin Intensity", "Cell Count", "Survival Curve"
        ])
        self.y_axis_combo2.currentTextChanged.connect(lambda: self.update_plot_metric(2))
        plot_controls2.addWidget(self.y_axis_combo2)
        plot_layout2.addLayout(plot_controls2)
        layout.addWidget(plot_container2)

        self.control_tabs.addTab(tab, "Plots_Metrics")

        # Ensure plots are drawn initially
        self.update_plot_metric(1)
        self.update_plot_metric(2)

    def update_plot_metric(self, player_num):
        if player_num == 1:
            player = self.player1
            figure = self.figure1
            canvas = self.canvas1
            y_axis_combo = self.y_axis_combo1
        else:
            player = self.player2
            figure = self.figure2
            canvas = self.canvas2
            y_axis_combo = self.y_axis_combo2
        y_metric = y_axis_combo.currentText()
        figure.clear()
        ax = figure.add_subplot(111)
        # Plot data for the selected player
        if hasattr(player, 'channel_images') and player.channel_images:
            values = self.calculate_plot_values(player, y_metric)
            timepoints = list(range(len(values)))
            if values:
                ax.plot(timepoints, values, label=f"Video {player_num} - {player.selected_uid}", marker='o')
        ax.set_title(f"{y_metric} Over Time")
    ...
    def setup_snr_metrics_tab(self):
        ...
        layout.addWidget(snr1_group)

        # Video 2 SNR Table (bottom)
        ...
        layout.addWidget(snr2_group)

        # Update button below both tables
    ...
    def setup_cell_flags_tab(self):
        ...
        # --- Cell Flags section (top, moved up) ---
        ...
        buttons_layout = QVBoxLayout()  # Stack buttons vertically
        ...
        self.download_video_btn.setStyleSheet("""
            QPushButton {
                background-color: #2196F3;
                color: white;
                border: none;
        ...
        """)
        buttons_layout.addWidget(self.download_video_btn)
        ...
        # --- Overlay Options section (top, moved up) ---
        ...
        self.show_boxes_checkbox.stateChanged.connect(self.toggle_boxes)
        self.show_text_checkbox.stateChanged.connect(self.toggle_text)
        self.show_contours_checkbox.stateChanged.connect(self.toggle_contours)
        self.show_base_checkbox.stateChanged.connect(self.toggle_base)

        # --- Now create side_controls and top_row ---
        side_controls = QWidget()
        side_controls_layout = QHBoxLayout(side_controls)  # Changed to QHBoxLayout for side-by-side
        side_controls_layout.setContentsMargins(0, 0, 0, 0)
        side_controls_layout.setSpacing(10)
        side_controls_layout.addWidget(cell_flags_group)
        side_controls_layout.addWidget(overlay_group)

        top_row = QHBoxLayout()
        top_row.setContentsMargins(0, 0, 0, 0)
        top_row.setSpacing(10)
        top_row.addWidget(self.video_container, 0, alignment=Qt.AlignTop | Qt.AlignLeft)  # No stretch for video
        top_row.addWidget(side_controls, 1, alignment=Qt.AlignTop)  # Stretch for side controls
        layout.addLayout(top_row)
    ...
    def set_video_data(self, player1_data, player2_data):
        ...
        if player1_data:
            ...
            print(f"[DEBUG] DualVideoWindow.set_video_data (player1): {len(images1)} images, {len(boxes1)} boxes, {len(channel_images1)} channel_images for UID {uid1}")
        ...
        if player2_data:
            ...
            print(f"[DEBUG] DualVideoWindow.set_video_data (player2): {len(images2)} images, {len(boxes2)} boxes, {len(channel_images2)} channel_images for UID {uid2}")
        ...
        if hasattr(self, 'update_plot_metric'):
            self.update_plot_metric(1)
            self.update_plot_metric(2)
    ...
    def previous_uid_for_player(self, player_id):
        if self.parent() and hasattr(self.parent(), 'previous_uid_for_player'):
            self.parent().previous_uid_for_player(player_id)

    def next_uid_for_player(self, player_id):
        if self.parent() and hasattr(self.parent(), 'next_uid_for_player'):
            self.parent().next_uid_for_player(player_id)
    ...
    def toggle_boxes(self, state):
        if hasattr(self, 'player1'):
            self.player1.toggle_boxes(state)
        if hasattr(self, 'player2'):
            self.player2.toggle_boxes(state)

    def toggle_text(self, state):
        if hasattr(self, 'player1'):
            self.player1.toggle_text(state)
        if hasattr(self, 'player2'):
            self.player2.toggle_text(state)
    
    def toggle_contours(self, state):
        if hasattr(self, 'player1'):
            self.player1.toggle_contours(state)
        if hasattr(self, 'player2'):
            self.player2.toggle_contours(state)

    def toggle_base(self, state):
        if hasattr(self, 'player1'):
            self.player1.toggle_base(state)
        if hasattr(self, 'player2'):
            self.player2.toggle_base(state)
...
class VideoWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        # ... earlier code in __init__ ...



        # Video display container with fixed aspect ratio and navigation buttons
        video_container = QWidget()
        video_container.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)  # Allow video to expand
        # video_container.setMaximumWidth(2000)  # Increased from 350 to 500
        video_layout = QHBoxLayout(video_container)
        video_layout.setContentsMargins(0, 0, 0, 0)
        video_layout.setSpacing(0)
        ...
        # --- Now create side_controls and top_row ---
        side_controls = QWidget()
        side_controls_layout = QVBoxLayout(side_controls)  # Changed to QHBoxLayout for side-by-side
        side_controls_layout.setContentsMargins(0, 0, 0, 0)
        side_controls_layout.setSpacing(10)
        side_controls_layout.addWidget(cell_flags_group)
        side_controls_layout.addWidget(overlay_group)
        # side_controls.setMaximumWidth(400)

        top_row = QHBoxLayout()
        top_row.setContentsMargins(0, 0, 0, 0)
        top_row.setSpacing(10)
        top_row.addWidget(video_container, 3, alignment=Qt.AlignTop | Qt.AlignLeft)  # No stretch for video
        top_row.addWidget(side_controls, 1.5, alignment=Qt.AlignTop)  # Stretch for side controls
        left_layout.addLayout(top_row, stretch=20)

        # ... rest of your left panel setup ...
        ...
        def previous_uid():
            ...
            if not parent:
                return
            if hasattr(parent, 'active_dataset') and parent.active_dataset == 2:
                uid_list = getattr(parent, 'uid_list2', [])
                current_index = getattr(parent, 'current_uid_index2', -1)
                if not uid_list or current_index == -1:
                    return
                new_index = (current_index - 1) % len(uid_list)
                parent.current_uid_index2 = new_index
                new_uid = uid_list[new_index]
                # Update table selection for UI feedback
                for row in range(parent.table2.rowCount()):
                    item = parent.table2.item(row, 0)
                    if item and item.text() == new_uid:
                        parent.table2.selectRow(row)
                        parent.table2.scrollToItem(item)
                        break
                # Directly load the video
                parent.selected_uid = new_uid
                parent.start_image_loading(new_uid, 2)
            else:
                uid_list = getattr(parent, 'uid_list', [])
                current_index = getattr(parent, 'current_uid_index', -1)
                if not uid_list or current_index == -1:
                    return
                new_index = (current_index - 1) % len(uid_list)
                parent.current_uid_index = new_index
                new_uid = uid_list[new_index]
                # Update table selection for UI feedback
                for row in range(parent.table1.rowCount()):
                    item = parent.table1.item(row, 0)
                    if item and item.text() == new_uid:
                        parent.table1.selectRow(row)
                        parent.table1.scrollToItem(item)
                        break
                # Directly load the video
                parent.selected_uid = new_uid
                parent.start_image_loading(new_uid, 1)
        ...
        # Only add the video label to the video area (no side arrows)
        self.video_label = VideoLabel()
        ...
        # left_layout.addWidget(video_container, alignment=Qt.AlignTop | Qt.AlignLeft)
         

        # Move playback controls group below the video
        ...
        self.next_frame_button.clicked.connect(self.next_frame)
        main_controls_layout.addWidget(self.next_frame_button)
        playback_layout.addLayout(main_controls_layout)
        ...
        # Channel Overlays (moved to left panel to match image)
        ...
        for row, (channel_key, (label_text, color)) in enumerate(fluor_channels.items()):
            ...
            # Connect the checkbox and slider to their respective methods
            cb.stateChanged.connect(lambda state, ch=channel_key: self.toggle_channel_overlay(ch))
            thresh_slider.valueChanged.connect(lambda value, ch=channel_key: self.update_channel_threshold(ch, value))

        channel_group.setLayout(channel_layout)
        left_layout.addWidget(channel_group)

        # Pixel Intensity Display (moved to left panel to match image)
        ...
        """)
        self.pixel_intensity_label.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
        left_layout.addWidget(self.pixel_intensity_label, stretch=0)
        ...
        # 4. Analysis Parameters and Colors side by side
        analysis_colors_row = QHBoxLayout()
        ...
        analysis_group.setLayout(analysis_layout)
        analysis_colors_row.addWidget(analysis_group)
        ...
        color_layout.addWidget(self.tar_color_picker)
        color_group.setLayout(color_layout)
        analysis_colors_row.addWidget(color_group)
        
        right_scroll_layout.addLayout(analysis_colors_row)

      

        # 6. SNR Table with minimum height
        legend_group = QGroupBox("SNR")
        legend_layout = QVBoxLayout()
        self.legend_table = QTableWidget(3, 7)
        ...
        self.legend_table.setMinimumHeight(150)  # Ensure minimum height
        legend_layout.addWidget(self.legend_table)
        
        # Add update SNR button
        update_snr_btn = QPushButton("Update SNR Metrics")
        update_snr_btn.clicked.connect(self.update_snr_table)
        update_snr_btn.setStyleSheet("""
            QPushButton {
                background-color: #2196F3;
                color: white;
        ...
        """)
        legend_layout.addWidget(update_snr_btn)
        
        legend_group.setLayout(legend_layout)
        right_scroll_layout.addWidget(legend_group)

        # 7. Nanowell Metrics Over Time (moved to right panel)
        ...
        right_scroll_layout.addWidget(plot_container)
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, "video_label"):
            # If the window is large, use 800x800 as minimum, else 250x250
            w, h = self.width(), self.height()
            if w > 1000 and h > 1000:
                self.video_label.setMinimumSize(900, 900)
            else:
                self.video_label.setMinimumSize(600, 600)

    def update_channel_threshold(self, channel, value):
            """Update channel threshold in the video window"""
            if hasattr(self, 'video_window'):
                self.video_window.channel_thresholds[channel] = value
                self.video_window.switch_images(self.video_window.image_index)
    ...
    def set_video_data(self, images, boxes, channel_images, uid, run_dirname, continuation=True):
        if continuation:
            ...
            self.boxes = boxes
            self.channel_images = channel_images
            # print(channel_images)  # Removed to avoid large array output
            # print(images)  # Removed to avoid large array output
            self.selected_uid = uid
            self.run_dirname = run_dirname
            self.image_index = 0
            print(
    ...
    def switch_images(self, image_index=-1):
        """
        Display the next frame with smooth blended RGB overlay,
        honoring current toggles for base, boxes, text, and channel thresholds.
        """
        if not self.channel_images or not self.images:
            return
        ...
        current = self.channel_images[self.image_index]
        if current is None:
                return

        # Extract raw channel images (low-gain & high-gain for threshold)
        ch0 = current.get('CH0', [np.zeros((500,500), np.uint8)])[0]
        ch1_data = current.get('CH1', [None, None])
        ch2_data = current.get('CH2', [None, None])
        ch3_data = current.get('CH3', [None, None])
        ch1, hi_ch1 = ch1_data[0], ch1_data[1]
        ch2, hi_ch2 = ch2_data[0], ch2_data[1]
        ch3, hi_ch3 = ch3_data[0], ch3_data[1]
        ...
        def norm(im):
            if im is None:
                return np.zeros_like(ch0, dtype=float)
            arr = im.astype(float)
        ...
        # Fetch thresholds from sliders
        t1 = self.channel_overlay_checkboxes['CH1']['slider'].value()
        t2 = self.channel_overlay_checkboxes['CH2']['slider'].value()
        t3 = self.channel_overlay_checkboxes['CH3']['slider'].value()

        # Build masks using high-intensity images
        mask1 = (hi_ch1 is not None) and (hi_ch1 >= t1)
        mask2 = (hi_ch2 is not None) and (hi_ch2 >= t2)
        mask3 = (hi_ch3 is not None) and (hi_ch3 >= t3)

        # Apply masks to fluorescence channels
        h1 = h1 * mask1
        h2 = h2 * mask2
        h3 = h3 * mask3

        # Determine which channels are enabled
        do_ch1 = self.channel_overlay_checkboxes['CH1']['checkbox'].isChecked()
        do_ch2 = self.channel_overlay_checkboxes['CH2']['checkbox'].isChecked()
        do_ch3 = self.channel_overlay_checkboxes['CH3']['checkbox'].isChecked()

        # Start blended image: either grayscale base or black
        if getattr(self, 'show_base', True):
            blended = np.stack([h0, h0, h0], axis=-1)
        else:
            h, w = h0.shape
            blended = np.zeros((h, w, 3), dtype=float)

        # Add each enabled channel
        if do_ch1:
            blended[..., 1] += 0.5 * h1  # green
        if do_ch2:
            blended[..., 0] += 0.5 * h2  # red
        if do_ch3:
            blended[..., 2] += 0.8 * h3  # blue

        # Clamp and convert back to uint8 RGB
        np.clip(blended, 0, 1, out=blended)
        rgb_overlay = (blended * 255).astype(np.uint8)

        # Convert to QPixmap
        h, w, _ = rgb_overlay.shape
        qimg = QImage(rgb_overlay.data, w, h, w*3, QImage.Format_RGB888)
        pixmap = QPixmap.fromImage(qimg)

        # Scale to square fill
        label_size = self.video_label.size()
        side = min(label_size.width(), label_size.height(), pixmap.width())
        scaled = pixmap.scaled(side, side, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        final = QPixmap(side, side)
        final.fill(QColor('#f0f0f0'))
        painter = QPainter(final)
        painter.drawPixmap((side - scaled.width())//2, (side - scaled.height())//2, scaled)

        # Overlay boxes and/or text
        painter.setRenderHint(QPainter.Antialiasing)
        painter.setRenderHint(QPainter.TextAntialiasing)
        if self.boxes and self.image_index < len(self.boxes):
            scale_x = scaled.width() / w
            scale_y = scaled.height() / h
            x_off = (side - scaled.width()) // 2
            y_off = (side - scaled.height()) // 2

            for category, color in (('eff', self.eff_color), ('tar', self.tar_color)):
                for box in self.boxes[self.image_index].get(category, []):
                    left, top, bw, bh = map(int, box)
                    lx = int(left * scale_x) + x_off
                    ly = int(top * scale_y) + y_off
                    lw = int(bw * scale_x)
                    lh = int(bh * scale_y)
                    # Draw box if enabled
                    if getattr(self, 'show_boxes', True):
                        painter.setPen(QPen(color, max(2, int(scale_x * 2))))
                        painter.drawRect(lx, ly, lw, lh)
                    # Draw text if enabled
                    if getattr(self, 'show_text', True):
                        painter.setPen(QPen(color))
                        painter.setFont(QFont('Arial', max(12, int(20 * scale_x))))
                        painter.drawText(QRect(lx, ly, lw, lh), Qt.AlignCenter, category[0].upper())

        painter.end()
        self.video_label.setPixmap(final)

    def blend_channels(ch0, ch1=None, ch2=None, ch3=None,
                    w1=0.5, w2=0.5, w3=0.5):
        """
        ch0, ch1, ch2, ch3: 2D numpy arrays (dtype=float or uint8).
        Returns a uint8 RGB image of shape (H, W, 3).
        Weights w1,w2,w3 control fluorescence contributions.
        """
        # 1) normalize all to float [0..1]
        def norm(im):
            im = im.astype(float)
        ...
        h0 = norm(ch0)
        # if any channel is missing, substitute zeros
        h1 = norm(ch1) if ch1 is not None else np.zeros_like(h0)
        h2 = norm(ch2) if ch2 is not None else np.zeros_like(h0)
        h3 = norm(ch3) if ch3 is not None else np.zeros_like(h0)

        # 2) stack into RGB base
        blended = np.stack([h0, h0, h0], axis=-1)  # [R,G,B] = [CH0,CH0,CH0]

        # 3) paint each fluor into its plane
        blended[..., 1] += w1 * h1   # green ? CH1
        blended[..., 0] += w2 * h2   # red   ? CH2
        blended[..., 2] += w3 * h3   # blue  ? CH3

        # 4) clamp and convert back to uint8
        np.clip(blended, 0, 1, out=blended)
    ...
    def toggle_channel_overlay(self, channel):
        """Toggle channel overlay in the video window"""
        # This method is called when channel overlay checkboxes are toggled
        # The actual overlay logic is handled in switch_images method
        self.switch_images(self.image_index)
    ...
    def toggle_base(self, state):
        self.show_base = (state == Qt.Checked)
        self.switch_images(self.image_index)
...
class QC_APP(QMainWindow):
    def __init__(self, args):
        ...
        self.default_path = "/cellchorus/data/d/clients/250620"
        ...
        print(args)
        self.mtp = 4000  # Increased to allow more frames
        
        # Initialize variables for dual video comparison
        self.dual_video_window = None

        # Add this attribute to QC_APP.__init__
        self.active_dataset = 1  # 1 or 2, default to 1
    ...
    def on_table_clicked(self, table_num):
        ...
        if table_num == 1:
            self.active_dataset = 1  # Track which dataset is active
            ...
            if len(selected_rows) == 1:
                ...
                if self.table1.item(selected_row_index, 0):
                    ...
                    if hasattr(self, 'uid_list') and self.table1_selected_uid in self.uid_list:
                        self.current_uid_index = self.uid_list.index(self.table1_selected_uid)
                    self.start_image_loading(self.table1_selected_uid, 1)
        elif table_num == 2:
            self.active_dataset = 2  # Track which dataset is active
            ...
            if len(selected_rows) == 1:
                ...
                if self.table2.item(selected_row_index, 0):
                    ...
                    if hasattr(self, 'uid_list2') and self.table2_selected_uid in self.uid_list2:
                        self.current_uid_index2 = self.uid_list2.index(self.table2_selected_uid)
    ...
    def start_image_loading(self, uid, table_num=1):
        ...
        # Connect to both progress and finished signals for single video
    ...
    def on_image_loading_finished(self, images, boxes, channel_images, uid, mtp):
        print(f"[DEBUG] QC_APP.on_image_loading_finished: {len(images)} images, {len(boxes)} boxes, {len(channel_images)} channel_images for UID {uid}")
        self.mtp = mtp if mtp != 0 else self.mtp
        print("UID ", uid)
        if uid == self.selected_uid:
            # Only update if we don't already have the correct number of frames
            current_frames = len(self.video_window.images) if hasattr(self.video_window, 'images') else 0
            if current_frames != len(images):
                print(f"[DEBUG] Updating video data: {current_frames} -> {len(images)} frames")
                ...
                # Update nanowell info label
                if hasattr(self.video_window, 'nanowell_info_label'):
                    self.video_window.nanowell_info_label.setText(f"Nanowell info:\nUID: {uid}")
                print("[DEBUG] Video window updated with complete data")
            else:
                print(f"[DEBUG] Video window already has correct number of frames ({len(images)}), skipping update")
        else:
            print(f"[DEBUG] Loaded UID {uid} does not match selected UID {self.selected_uid}; ignoring update")

    def on_image_loading_progress(self, progress, images, boxes, channel_images, uid):
        print(f"[DEBUG] on_image_loading_progress called for UID: {uid} with {len(images)} images, progress: {progress}")
        # Only update if the loaded UID matches the currently selected one.
        if uid == self.selected_uid:
            # Handle initial batch (progress == 30) - this starts playback immediately
            if progress == 30:
                # First batch - start playback immediately
                if self.video_window.timer.isActive():
                    print("[DEBUG] Timer is active; stopping timer before initial data update")
                    self.video_window.timer.stop()
                
                # Clear previous data and set new initial data
                self.video_window.images = []
                self.video_window.boxes = []
                self.video_window.channel_images = []
                print(f"[DEBUG] Setting initial batch: {len(images)} images")

                # Update video window with initial data
                self.video_window.set_video_data(
                    images, boxes, channel_images, uid, self.run_dirname, False)
                ...
                print("[DEBUG] Initial video window updated and playback started")
            else:
                # Unknown progress value - treat as initial batch
                print(f"[DEBUG] Unknown progress value {progress}, treating as initial batch")
                if self.video_window.timer.isActive():
                    self.video_window.timer.stop()
    ...
    def load_run_file(self, table_num):
        if table_num == 1:
            ...
            if os.path.exists(sec_dirname):
                ...
                if csv_file_basenames:
                    self.open_run_file(csv_file_basenames[0], 1)
            else:
                self.dropdown1.clear()
            ...
            if os.path.exists(sec_dirname):
                ...
                # Auto-load the first file if available
                if csv_file_basenames:
                    self.open_run_file(csv_file_basenames[0], 2)
            else:
                self.dropdown2.clear()
                print(f"Directory not found: {sec_dirname}")
    ...
    def load_csv_to_table(self, filename, table_num):
        ...
        with open(filename, 'r', newline='') as csvfile:
            ...
            if table_num == 1:
                ...
                # --- Single video mode support ---
                self.uid_list = uid_list
                self.current_uid_index = 0 if uid_list else -1
            else:
        ...
        if hasattr(self, 'table1'):
            ...
            for row in range(self.table1.rowCount()):
                ...
                if item and item.text() == uid:
                    ...
                    # --- Single video mode: update current_uid_index and always load video ---
                    if hasattr(self, 'uid_list') and uid in self.uid_list:
                        self.current_uid_index = self.uid_list.index(uid)
                        print(f"[DEBUG] select_uid: Loading video for UID {uid} (single video mode)")
                        self.start_image_loading(uid, 1)
                    # Do not return here; always call start_image_loading
            ...
            for row in range(self.table_widget.rowCount()):
                ...
                if item and item.text() == uid:
                    ...
                    if hasattr(self, 'uid_list') and uid in self.uid_list:
                        ...
                        print(f"[DEBUG] select_uid: Loading video for UID {uid} (single video mode)")
                        self.start_image_loading(uid, 1)
                    # Do not break here; always call start_image_loading
    ...
    def load_video_for_player(self, uid, run_dirname, player_num):
        ...
        try:
            ...
            # Connect to completion handler and progress handler
            loader.finished.connect(self.on_dual_video_loading_finished)
            loader.progress.connect(self.on_dual_video_loading_progress)
    ...
    def on_dual_video_loading_progress(self, progress, images, boxes, channel_images, uid):
        print(f"[DEBUG] QC_APP.on_dual_video_loading_progress: {len(images)} images for UID {uid}, progress {progress}")
        try:
            ...
            if self.dual_video_window:
                ...
                player = self.dual_video_window.player1 if player_num == 1 else self.dual_video_window.player2
                
                if progress == 30:
                    # First batch - start playback immediately
                    print(f"[DEBUG] Setting initial batch for player {player_num}: {len(images)} images")
                    player.set_video_data(images, boxes, channel_images, uid, getattr(loader, 'run_dirname', ''), False)
                else:
                    # Unknown progress value - treat as initial batch
                    print(f"[DEBUG] Unknown progress value {progress} for player {player_num}, treating as initial batch")
                    player.set_video_data(images, boxes, channel_images, uid, getattr(loader, 'run_dirname', ''), False)
                        
        except Exception as e:
            print(f"Error handling dual video loading progress: {e}")

    def on_dual_video_loading_finished(self, images, boxes, channel_images, uid, mtp):
        print(f"[DEBUG] QC_APP.on_dual_video_loading_finished: {len(images)} images, {len(boxes)} boxes, {len(channel_images)} channel_images for UID {uid}")
        try:
            # Get the loader to determine which player this is for
            loader = self.sender()
            player_num = getattr(loader, 'player_num', 1)
    ...
    def toggle_boxes(self, state):
        """Toggle boxes overlay in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.show_boxes = state == Qt.Checked
            self.video_window.switch_images(self.video_window.image_index)

    def toggle_text(self, state):
        """Toggle text overlay in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.show_text = state == Qt.Checked
            self.video_window.switch_images(self.video_window.image_index)

    def toggle_contours(self, state):
        """Toggle contours overlay in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.show_contours = state == Qt.Checked
            self.video_window.switch_images(self.video_window.image_index)

    def toggle_base(self, state):
        """Toggle base image overlay in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.show_base = state == Qt.Checked
            self.video_window.switch_images(self.video_window.image_index)

    def toggle_channel_overlay(self, channel):
        """Toggle channel overlay in the video window"""
        if hasattr(self, 'video_window'):
            checkbox = self.channel_overlay_checkboxes[channel]['checkbox']
            is_checked = checkbox.isChecked()
            # Update the video window's channel overlay checkboxes
            if not hasattr(self.video_window, 'channel_overlay_checkboxes'):
                self.video_window.channel_overlay_checkboxes = {}
            if channel not in self.video_window.channel_overlay_checkboxes:
                self.video_window.channel_overlay_checkboxes[channel] = {'checkbox': checkbox}
            else:
                self.video_window.channel_overlay_checkboxes[channel]['checkbox'] = checkbox
            self.video_window.switch_images(self.video_window.image_index)
    ...
    def update_snr_table(self):
        """Update the Signal Metrics table with current channel data"""
        # Check if legend_table exists
        if not hasattr(self, 'legend_table'):
            print("[DEBUG] legend_table not yet created, skipping SNR update")
            return
            
        if not hasattr(self, 'video_window') or not self.video_window.channel_images:
            return
        
        if self.video_window.image_index >= len(self.video_window.channel_images):
            return
        
        current_images = self.video_window.channel_images[self.video_window.image_index]
        if current_images is None:
            return
        ...
        for channel in channels:
            if channel in current_images:
                channel_data = current_images[channel]
                signal, background, noise, snr, sbr, cnr = self.video_window.calculate_snr(channel_data)
    ...
    def previous_frame(self):
        """Go to previous frame in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.previous_frame()
            if hasattr(self, 'legend_table'):
                self.update_snr_table()

    def next_frame(self):
        """Go to next frame in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.next_frame()
            if hasattr(self, 'legend_table'):
                self.update_snr_table()

    def toggle_playback(self):
        """Toggle playback in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.toggle_playback()

    def slider_value_changed(self):
        """Handle slider value change in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.slider_value_changed()

    def slider_value_released(self):
        """Handle slider value release in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.slider_value_released()
            if hasattr(self, 'legend_table'):
                self.update_snr_table()

    def change_playback_speed(self, value):
        """Change playback speed in the video window"""
        if hasattr(self, 'video_window'):
            self.video_window.change_playback_speed(value)
