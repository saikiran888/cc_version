...
class TIMING_secondary:
    ...
    def secondary(self, DInt, EDInt, ContInt, TimInt, Pix):
        ...
        for i, k in enumerate(['tot', 'death', 'no_death']):
            df_summary.iat[2 + i, 1] = k
            df_summary.iat[2 + i, 2] = df_0E_1T_analysis[k].shape[0]
        df_summary.iat[5, 1] = "% death"
        if df_0E_1T_analysis['tot'].shape[0] != 0:
            df_summary.iat[5, 2] = df_0E_1T_analysis['death'].shape[0] / \
                df_0E_1T_analysis['tot'].shape[0]

        # 1E0T organizing
        df_summary.iat[7, 1] = '1E 0T'
        for i, k in enumerate(['tot', 'death', 'no_death']):
            df_summary.iat[8 + i, 1] = k
            df_summary.iat[8 + i, 2] = df_1E_0T_analysis[k].shape[0]
        df_summary.iat[11, 1] = "% death"
        if df_1E_0T_analysis['tot'].shape[0] != 0:
            df_summary.iat[11, 2] = df_1E_0T_analysis['death'].shape[0] / \
                df_1E_0T_analysis['tot'].shape[0]

        # 1E1T organizing
        df_summary.iat[13, 1] = '1E 1T'
        for i, k in enumerate(['tot', 'valid']):
            df_summary.iat[14 + i, 1] = k
            df_summary.iat[14 + i, 2] = df_1E_1T_analysis[k].shape[0]

        for i, k in enumerate(['valid_contact', 'valid_contact_kill', 'valid_contact_no_kill']):
            df_summary.iat[14 + i, 4] = k
            df_summary.iat[14 + i, 5] = df_1E_1T_analysis[k].shape[0]
        df_summary.iat[17, 4] = "% kill"
        if df_1E_1T_analysis['valid_contact'].shape[0] != 0:
            df_summary.iat[17, 5] = df_1E_1T_analysis['valid_contact_kill'].shape[0] / \
        ...
        for i, k in enumerate(['valid_no_contact', 'valid_no_contact_death', 'valid_no_contact_no_death']):
            df_summary.iat[14 + i, 7] = k
            df_summary.iat[14 + i, 8] = df_1E_1T_analysis[k].shape[0]
        df_summary.iat[17, 7] = "% death"
        if df_1E_1T_analysis['valid_no_contact'].shape[0] != 0:
            df_summary.iat[17, 8] = df_1E_1T_analysis['valid_no_contact_death'].shape[0] / \
        ...
        # }
        # 1E2T organizing
        df_summary.iat[19, 1] = '1E 2T'
        for i, k in enumerate(['tot', 'valid']):
            df_summary.iat[20 + i, 1] = k
            df_summary.iat[20 + i, 2] = df_1E_2T_analysis[k].shape[0]

        for i, k in enumerate(['valid_2_contact', 'valid_2_contact_2_kill', 'valid_2_contact_1_kill', 'valid_2_contact_no_kill']):
            df_summary.iat[20 + i, 4] = k
            df_summary.iat[20 + i, 5] = df_1E_2T_analysis[k].shape[0]
        df_summary.iat[23, 4] = "% no kill"
        if df_1E_2T_analysis['valid_2_contact'].shape[0] != 0:
            df_summary.iat[23, 5] = df_1E_2T_analysis['valid_2_contact_no_kill'].shape[0] / \
        ...
        for i, k in enumerate(['valid_1_contact', 'valid_1_contact_1_kill', 'valid_1_contact_no_kill']):  # 3
            df_summary.iat[20 + i, 8] = k
            df_summary.iat[20 + i, 9] = df_1E_2T_analysis[k].shape[0]
        df_summary.iat[23, 8] = "% no kill"
        if df_1E_2T_analysis['valid_1_contact'].shape[0] != 0:
            df_summary.iat[23, 9] = df_1E_2T_analysis['valid_1_contact_no_kill'].shape[0] / \
        ...
        for i, k in enumerate(['valid_no_contact', 'valid_no_contact_2_death', 'valid_no_contact_1_death', 'valid_no_contact_no_death']):
            df_summary.iat[20 + i, 12] = k
            df_summary.iat[20 + i, 13] = df_1E_2T_analysis[k].shape[0]
        df_summary.iat[23, 12] = "% no death"
        if df_1E_2T_analysis['valid_no_contact'].shape[0] != 0:
            df_summary.iat[23, 13] = df_1E_2T_analysis['valid_no_contact_no_death'].shape[0] / \
        ...
        with pd.ExcelWriter(os.path.join(home_directory, "event_breakdown.xlsx")) as writer:
            ...
            # df_0E_1T.to_excel(writer, sheet_name="Table_0E1T", index=False)
            # df_1E_0T.to_excel(writer, sheet_name="Table_1E0T", index=False)
            # df_1E_1T.to_excel(writer, sheet_name="Table_1E1T", index=False)
            # df_1E_2T.to_excel(writer, sheet_name="Table_1E2T", index=False)
            # df_1E_3T.to_excel(writer, sheet_name="Table_1E3T", index=False)
            ...
            for k, v in df_0E_1T_analysis.items():
                ...
                v.to_excel(
                    writer, sheet_name=f"0E_1T_{k_name} (n={v.shape[0]})", index=False)
            for k, v in df_1E_0T_analysis.items():
                k_name = k.replace("contact", "c").replace("valid", "v")
                v.to_excel(
                    writer, sheet_name=f"1E_0T_{k_name} (n={v.shape[0]})", index=False)
            for k, v in df_1E_1T_analysis.items():
                k_name = k.replace("contact", "c").replace("valid", "v")
                v.to_excel(
                    writer, sheet_name=f"1E_1T_{k_name} (n={v.shape[0]})", index=False)
            for k, v in df_1E_2T_analysis.items():
                k_name = k.replace("contact", "c").replace("valid", "v")
                v.to_excel(
                    writer, sheet_name=f"1E_2T_{k_name} (n={v.shape[0]})", index=False)
            for k, v in df_1E_3T_analysis.items():
                k_name = k.replace("contact", "c").replace("valid", "v")
                v.to_excel(
                    writer, sheet_name=f"1E_3T_{k_name} (n={v.shape[0]})", index=False)
        ...
        def format_summary_excel(output_path):
            ...
            def find_value_in_section(section_label, label, max_rows=20):
            ...
            row_ptr += 1
            tot = find_value_in_section('0E 1T', 'tot')
            alive = find_value_in_section('0E 1T', 'no_death')
            dead = find_value_in_section('0E 1T', 'death')
            alive_fraction = round(alive / tot, 4) if tot else 0
            dead_fraction = round(dead / tot, 4) if tot else 0
            block = [
                ['total', tot, '', ''],
                ['', 'alive', alive, alive_fraction],
                ['', 'dead', dead, dead_fraction]
            ]
            ...
            ws_out.cell(row=row_ptr, column=2).fill = yellow
            row_ptr += 1
            tot = find_value_in_section('1E 0T', 'tot')
            alive = find_value_in_section('1E 0T', 'no_death')
            dead = find_value_in_section('1E 0T', 'death')
            alive_fraction = round(alive / tot, 4) if tot else 0
            dead_fraction = round(dead / tot, 4) if tot else 0
            block = [
                ['total', tot, '', ''],
            ...
            ws_out.cell(row=row_ptr, column=2).fill = yellow
            row_ptr += 1
            tot = find_value_in_section('1E 1T', 'tot')
            valid = find_value_in_section('1E 1T', 'valid')
            ...
            row_ptr += len(block)
            # Contact breakdown
            valid_no_contact = find_value_in_section('1E 1T', 'valid_no_contact') or 0
            valid_contact = find_value_in_section('1E 1T', 'valid_contact') or 0
            ...
            valid_no_contact_no_death = find_value_in_section('1E 1T', 'valid_no_contact_no_death')
            valid_no_contact_death = find_value_in_section('1E 1T', 'valid_no_contact_death')
            ...
            valid_contact_no_kill = find_value_in_section('1E 1T', 'valid_contact_no_kill')
            valid_contact_kill = find_value_in_section('1E 1T', 'valid_contact_kill')
            ...
            ws_out.cell(row=row_ptr, column=2).fill = yellow
            row_ptr += 1
            tot = find_value_in_section('1E 2T', 'tot')
            valid = find_value_in_section('1E 2T', 'valid')
            invalid = tot - valid if tot is not None and valid is not None else None
            valid_fraction = round(valid / tot, 4) if tot else 0
            invalid_fraction = round(invalid / tot, 4) if tot else 0
            block = [
            ...
            row_ptr += len(block)
            # Contact breakdown
            valid_no_contact = find_value_in_section('1E 2T', 'valid_no_contact')
            valid_1_contact = find_value_in_section('1E 2T', 'valid_1_contact')
            valid_2_contact = find_value_in_section('1E 2T', 'valid_2_contact')
            ...
            row_ptr += 1
            # 0 contact block
            valid_no_contact_0_killed = find_value_in_section('1E 2T', 'valid_no_contact')
            valid_no_contact_1_killed = find_value_in_section('1E 2T', 'valid_no_contact_1_death')
            valid_no_contact_2_killed = find_value_in_section('1E 2T', 'valid_no_contact_2_death')
            ...
            # 1 contact block
            valid_1_contact_no_kill = find_value_in_section('1E 2T', 'valid_1_contact_no_kill')
            valid_1_contact_1_kill = find_value_in_section('1E 2T', 'valid_1_contact_1_kill')
            ...
            # 2 contact block
            valid_2_contact_1_kill = find_value_in_section('1E 2T', 'valid_2_contact_1_kill')
            valid_2_contact_2_kill = find_value_in_section('1E 2T', 'valid_2_contact_2_kill')
