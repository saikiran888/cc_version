...
def save_crops_bucketed(result: Dict, images: Dict[int, np.ndarray], input_root: Path, base_out_dir: Path, pattern: re.Pattern):
    ...
    # --- 1) temporal consistency as before ---
    ...
    # --- 2) build the grid-based sorted order over *all* boxes ---
    ...
    # --- 4) now crop in that exact sequence ---
    bucket = get_bucket_name(scene_id)
    img8, img16, vis_dir, debug_dir = get_bucket_dirs(bucket, base_out_dir)
    for d in (img8.parent, img16.parent, vis_dir, debug_dir):
    ...
    for well_num, (idx, (x1,y1,x2,y2)) in enumerate(to_crop, start=1):
        for ch in CHANNELS:
            ...
            # 16-bit
            out16 = img16 / fname
            cv2.imwrite(str(out16), cv2.resize(crop, CROP_SIZE), [cv2.IMWRITE_TIFF_COMPRESSION,1])
            print_and_log(f"Saved 16-bit crop: {out16}")

            # 8-bit
            norm8   = cv2.normalize(crop, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
            out8    = img8  / fname
            cv2.imwrite(str(out8), cv2.resize(norm8, CROP_SIZE), [cv2.IMWRITE_TIFF_COMPRESSION,1])
            print_and_log(f"Saved 8-bit crop: {out8}")

            # vis
            outvis  = vis_dir / fname
            cv2.imwrite(str(outvis), cv2.resize(crop, CROP_SIZE), [cv2.IMWRITE_TIFF_COMPRESSION,1])
