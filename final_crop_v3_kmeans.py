...
def save_crops_bucketed(result: Dict, images: Dict[int, np.ndarray], input_root: Path, base_out_dir: Path, pattern: re.Pattern):
    ...
    if SAVE_DEBUG_IMAGE and images.get(1) is not None:
        ...
        # --- Robust grid assignment using k-means clustering ---
        if len(boxes) > 0:
            centers = np.array([[(box[0] + box[2]) / 2, (box[1] + box[3]) / 2] for box in boxes])
            n_grid = 6  # 6x6 grid
            # Cluster x and y separately
            kmeans_x = KMeans(n_clusters=n_grid, random_state=0).fit(centers[:, 0].reshape(-1, 1))
            kmeans_y = KMeans(n_clusters=n_grid, random_state=0).fit(centers[:, 1].reshape(-1, 1))
            # Get sorted cluster centers
            x_centers = np.sort(kmeans_x.cluster_centers_.flatten())
            y_centers = np.sort(kmeans_y.cluster_centers_.flatten())
            # Assign each well to nearest x and y cluster center
            grid_positions = []
            for i, (cx, cy) in enumerate(centers):
                col = np.argmin(np.abs(x_centers - cx))
                row = np.argmin(np.abs(y_centers - cy))
                grid_positions.append((row, col, i))
            # Sort by column, then row (column-major order)
        ...
        for idx, ((x1,y1,x2,y2), cls) in enumerate(zip(boxes, classes)):
            ...
            # Draw detection box with class color and thicker lines
            cv2.rectangle(dbg, (int(x1),int(y1)), (int(x2),int(y2)), color, 3)
            
            # Draw large well number and grid position at the top-left corner
            sequence_label = f"#{well_num:02d}"
            grid_label_large = f"C{row}R{col}"
            
            # Background rectangle for better text visibility
            text_bg_color = (0, 0, 0)  # Black background
            text_fg_color = (255, 255, 255)  # White text
            
            # Draw sequence number (cropping order) - large and prominent
            cv2.putText(dbg, sequence_label, (int(x1)+5, int(y1)+25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1.2, text_fg_color, 3)
            cv2.putText(dbg, sequence_label, (int(x1)+5, int(y1)+25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 2)
            
            # Draw row/column label - large and clear
            cv2.putText(dbg, grid_label_large, (int(x1)+5, int(y1)+55), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1.0, text_fg_color, 3)
            cv2.putText(dbg, grid_label_large, (int(x1)+5, int(y1)+55), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
            
            # Draw coordinates at the bottom-right corner with a background for readability
            coord_text = f"({int(center_x)},{int(center_y)})"
            coord_x = int(x2)-120
            coord_y = int(y2)-10
            (tw, th), _ = cv2.getTextSize(coord_text, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 3)
            # Draw filled rectangle as background
            cv2.rectangle(dbg, (coord_x-5, coord_y-th-5), (coord_x+tw+5, coord_y+5), (0,0,0), -1)
            # Draw the coordinates text
            cv2.putText(dbg, coord_text, (coord_x, coord_y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,255), 3)
            cv2.putText(dbg, coord_text, (coord_x, coord_y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            
            # Draw center point with larger circle
            cv2.circle(dbg, (int(center_x), int(center_y)), 4, color, -1)
            cv2.circle(dbg, (int(center_x), int(center_y)), 4, (255, 255, 255), 1)
            ...
            if cls != INVALID_CLASS:
                ...
                if was_invalid:
                    cv2.putText(dbg, "✓", (int(x2)-30, int(y2)-10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,0), 3)
        
        # Add summary statistics with larger text
        valid_count = sum(1 for c in classes if c != INVALID_CLASS)
        ...
        stats_text = [
            ...
            f"Cropping Sequence: 1-{len([c for c in classes if c != INVALID_CLASS])}"
        ]
        y_offset = 40
        ...
        # Add cropping sequence information
        sequence_y = y_offset + 20
        sequence_title = "CROPPING SEQUENCE:"
        cv2.putText(dbg, sequence_title, (10, sequence_y),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 3)
        cv2.putText(dbg, sequence_title, (10, sequence_y),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,0), 1)
        sequence_y += 30
        
        # Show the first few and last few wells in sequence
        valid_wells = [well_numbers.get(idx, idx+1) for idx, cls in enumerate(classes) if cls != INVALID_CLASS]
        if valid_wells:
            if len(valid_wells) <= 10:
                sequence_text = f"Order: {', '.join(map(str, valid_wells))}"
            else:
                first_few = valid_wells[:5]
                last_few = valid_wells[-5:]
                sequence_text = f"Order: {', '.join(map(str, first_few))} ... {', '.join(map(str, last_few))}"
            
            cv2.putText(dbg, sequence_text, (10, sequence_y),
