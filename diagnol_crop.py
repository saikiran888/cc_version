...
def save_crops_bucketed(result: Dict, images: Dict[int, np.ndarray], input_root: Path, base_out_dir: Path, pattern: re.Pattern):
    ...
    # --- VALIDITY CHECK BASED ON CENTER-TO-CORNER DISTANCES ---
    for idx, box in enumerate(boxes):
        x1, y1, x2, y2 = map(int, box)
        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
        corners = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
        distances = [np.sqrt((cx - corner_x) ** 2 + (cy - corner_y) ** 2) for (corner_x, corner_y) in corners]
        cls = classes[idx]
        if cls == 1:  # Diamond
            if all(d > 148 for d in distances):
                classes[idx] = 1  # Valid Diamond
            else:
                classes[idx] = 4  # Invalid
        elif cls == 0:  # Square
            if all(d > 115 for d in distances):
    ...
    if SAVE_DEBUG_IMAGE and images.get(1) is not None:
        ...
        for idx, box in enumerate(boxes):
            ...
            # Add center-to-corner distance measurements
            ...
            for i, (corner_x, corner_y) in enumerate(corners):
                ...
                cv2.line(dbg, (cx, cy), (corner_x, corner_y), (255, 255, 0), 2)  # Thicker yellow lines
                
                # Calculate midpoint for text placement
                mid_x = (cx + corner_x) // 2
                mid_y = (cy + corner_y) // 2
                
                # Add distance measurement text with better visibility
                distance_text = f"{distance:.0f}"  # Remove decimal places for cleaner display
                text_size = cv2.getTextSize(distance_text, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]  # Larger font, thicker
                
                # Larger background rectangle for text with padding
                padding = 4
                text_bg_x1 = mid_x - text_size[0]//2 - padding
                text_bg_y1 = mid_y - text_size[1]//2 - padding
                text_bg_x2 = mid_x + text_size[0]//2 + padding
                text_bg_y2 = mid_y + text_size[1]//2 + padding
                
                # Draw a more prominent background (dark blue with white border)
                cv2.rectangle(dbg, (text_bg_x1, text_bg_y1), (text_bg_x2, text_bg_y2), (0, 0, 0), -1)  # Black fill
                cv2.rectangle(dbg, (text_bg_x1, text_bg_y1), (text_bg_x2, text_bg_y2), (255, 255, 255), 2)  # White border
                
                # Draw text with better contrast
                cv2.putText(dbg, distance_text, (mid_x - text_size[0]//2, mid_y + text_size[1]//2),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)  # Cyan text, thicker
    ...
    for well_num, (idx, (x1,y1,x2,y2)) in enumerate(to_crop, start=1):
        for ch in CHANNELS:
            ...
            out16 = w16 / fname
            r16   = cv2.resize(crop, CROP_SIZE, interpolation=cv2.INTER_AREA)
            cv2.imwrite(str(out16), r16, [cv2.IMWRITE_TIFF_COMPRESSION,1])
            ...
            r8    = cv2.resize(norm8, CROP_SIZE, interpolation=cv2.INTER_AREA)
            out8  = w8 / fname
            cv2.imwrite(str(out8), r8, [cv2.IMWRITE_TIFF_COMPRESSION,1])
            ...
            outv = wv / fname
            cv2.imwrite(str(outv), r16, [cv2.IMWRITE_TIFF_COMPRESSION,1])
