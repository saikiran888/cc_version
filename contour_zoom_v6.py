...
def smoothed_contour(contour, smooth_strength=5):
    # contour: OpenCV format (Nx1x2)
    if len(contour) < 4:
        return contour
    x = [pt[0][0] for pt in contour]
    y = [pt[0][1] for pt in contour]
    try:
        tck, u = splprep([x, y], s=smooth_strength)
        u_new = np.linspace(0, 1, len(contour)*5)
...
# Edge-based refinement functions (move to top-level)
...
def refine_mask_with_edges(mask, edge_map):
    ...
    mask_boundary = cv2.Canny(mask.astype(np.uint8)*255, 20, 80)
    refined_mask = mask.copy()
    overlap = (dilated_edges > 0) & (mask_boundary > 0)
    refined_mask[overlap] = 1
...
class SamWorker(QObject):
    ...
    def __init__(self, checkpoint, model_type, device):
        ...
        self.sam         = None
        self.predictor   = None
        self.last_custom_checkpoint = None  # Cache for custom checkpoint path
    ...
    def loadModel(self):
        ...
        # For SAM predictor, we need to use single GPU to avoid DataParallel issues
        # The model will still benefit from multi-GPU memory and other optimizations
        if torch.cuda.device_count() > 1:
            print(f"Using GPU 0 for SAM model (multi-GPU available: {torch.cuda.device_count()} GPUs)")
            # Use the first available GPU for the predictor
            self.device = "cuda:0"
    ...
    def loadCustomCheckpoint(self, checkpoint_path):
        ...
        if self.last_custom_checkpoint == checkpoint_path:
            print(f"Custom checkpoint {checkpoint_path} already loaded, skipping...")
            return True
        ...
        try:
            ...
            self.sam.eval()
            self.last_custom_checkpoint = checkpoint_path  # Cache the path
    ...
    def resetToDefaultModel(self):
        """Reset to the default model and clear custom checkpoint cache"""
        if self.last_custom_checkpoint is not None:
            print("Resetting to default model...")
            ...
            self.sam.to(self.device)
            self.predictor = SamPredictor(self.sam)
            self.last_custom_checkpoint = None
            print("Reset to default model complete")
...
class VideoPredictionWorker(QObject):
    progress = pyqtSignal(int, str)  # frame_number, status_message
    finished = pyqtSignal(bool, str)  # success, message
    frame_complete = pyqtSignal(int)  # frame_number
    
    def __init__(self, sam_worker, base_path, block, nanowell, current_frame, 
                 boxes, frames_to_process, use_custom_model, custom_checkpoint,
                  gaussian_sigma, overlay_channels, corrected_label=None):
        super().__init__()
        ...
        self.tracking_histories = [[] for _ in range(len(boxes))]
        self.corrected_label = corrected_label
        
        # Use secondary GPU for image processing if available
        if torch.cuda.device_count() > 1:
            self.secondary_device = "cuda:1"
            print(f"Using secondary GPU ({self.secondary_device}) for image processing")
        else:
    ...
    def save_contours_as_freehand(self, contours, block, nanowell, frame, mask_idx):
        """Save extracted contours as FreeHand masks."""
        ...
        H, W = 281, 281  # Assuming standard image size
    ...
    def update_bbox_from_mask_robust(self, mask, original_bbox, frame_idx, tracking_history=None):
        """Enhanced bounding box update with improved movement handling."""
        ...
        # IMPROVED: More conservative sizing strategy
        # Use detected size as base, but be more conservative with large movements
        if movement_magnitude > 30:  # Reduced threshold for "large" movement
            # For large movements: use more of original size to avoid capturing neighbors
            # This prevents the box from growing too large when cells move
            adaptive_w = int(0.6 * orig_w + 0.4 * w)  # More weight on original size
            adaptive_h = int(0.6 * orig_h + 0.4 * h)
            # Minimal padding for large movements to avoid capturing neighbors
            padding = 5
        else:
            # Normal movement: balanced approach
            adaptive_w = int(0.5 * orig_w + 0.5 * w)
            adaptive_h = int(0.5 * orig_h + 0.5 * h)
            padding = 8
        
        # IMPROVED: More conservative size limits
        # Keep boxes smaller to avoid capturing neighboring cells
        max_size = 80 if movement_magnitude > 30 else 70  # Reduced from 150/100
        adaptive_w = min(adaptive_w + padding, max_size)
        adaptive_h = min(adaptive_h + padding, max_size)
        
        # IMPROVED: Ensure minimum size for detection
        min_size = 30
        adaptive_w = max(adaptive_w, min_size)
        adaptive_h = max(adaptive_h, min_size)
    ...
    def predict_bbox_from_motion(self, tracking_history, current_frame):
        ...
        # IMPROVED: Use more recent velocity data for better prediction
        # Calculate velocity from the most recent frames (last 3 frames)
        recent_history = tracking_history[-3:] if len(tracking_history) >= 3 else tracking_history
        velocities = []
        ...
        # IMPROVED: Weight recent velocities more heavily
        # Give more weight to recent movements for better sudden movement handling
        weights = np.linspace(0.5, 1.0, len(velocities))  # Recent movements get higher weight
        weighted_vx = np.average([v[0] for v in velocities], weights=weights)
        weighted_vy = np.average([v[1] for v in velocities], weights=weights)
        
        # IMPROVED: Add acceleration prediction for sudden movements
        if len(velocities) >= 2:
            # Calculate acceleration (change in velocity)
            recent_vx = [v[0] for v in velocities[-2:]]
            recent_vy = [v[1] for v in velocities[-2:]]
            accel_x = recent_vx[1] - recent_vx[0] if len(recent_vx) > 1 else 0
            accel_y = recent_vy[1] - recent_vy[0] if len(recent_vy) > 1 else 0
            
            # Apply acceleration to prediction
            weighted_vx += accel_x * 0.5  # Scale acceleration effect
            weighted_vy += accel_y * 0.5
        ...
        predicted_center_x = last_center[0] + weighted_vx * frames_since_last
        predicted_center_y = last_center[1] + weighted_vy * frames_since_last
        
        # Use last known bbox size
        bbox_w = last_bbox[2] - last_bbox[0]
        bbox_h = last_bbox[3] - last_bbox[1]
        
        # IMPROVED: Ensure predicted bbox doesn't get too large
        max_size = 80
        bbox_w = min(bbox_w, max_size)
        bbox_h = min(bbox_h, max_size)
    ...
    @pyqtSlot()
    def run(self):
        """Main video prediction loop in background thread."""
        try:
            # Handle model loading with caching
            if self.use_custom_model:
                if not self.sam_worker.loadCustomCheckpoint(self.custom_checkpoint):
                    self.finished.emit(False, "Failed to load custom checkpoint")
                    return
            else:
                # Reset to default model if we were using custom model before
                if self.sam_worker.last_custom_checkpoint is not None:
                    self.sam_worker.resetToDefaultModel()

            # Process each frame
            for frame_offset in range(0, self.frames_to_process):
                target_frame = self.current_frame + frame_offset
                self.progress.emit(frame_offset, f"Processing frame {target_frame}...")

                # Load the target frame image
                ch0_path = self.get_image_path(self.block, self.nanowell, str(target_frame), 'CH0')
                ...
                frame_overlay = self.create_fluorescence_overlay_for_frame(self.block, self.nanowell, str(target_frame))
                ...
                try:
                    self.sam_worker.predictor.set_image(frame_overlay)
                except Exception as e:
                ...
                for box_idx, box in enumerate(self.boxes):
                    ...
                    try:
                        # Get mask prediction
                        masks, scores, logits = self.sam_worker.predictor.predict(
                            box=sam_box,
                ...
                # Save masks for this frame with label-based merging
                sam_dir = os.path.join(
                    self.base_path, self.block, "Masks", "SAM",
                    f"imgNo{self.nanowell}CH0", f"t{target_frame}"
                )
                os.makedirs(sam_dir, exist_ok=True)
                
                # 1. Load existing masks from the current mask type (e.g., MRCNN) for this frame
                print(f"\n[VIDEO DEBUG] Frame {target_frame}")
                current_mask_paths = [
                    os.path.join(sam_dir, f)
                    for f in os.listdir(sam_dir)
                    if f.endswith('.npy')
                ]
                current_labels = []
                current_masks = []
                print(f"[VIDEO DEBUG] Loading existing masks for frame {target_frame} (mask type: SAM):")
                print(f"[VIDEO DEBUG] Current mask paths: {current_mask_paths}")
                for p in current_mask_paths:
                    try:
                        arr = np.load(p)
                        print(f"[VIDEO DEBUG] Loaded {os.path.basename(p)}: shape={arr.shape}, unique={np.unique(arr)}")
                        current_masks.append(arr)
                        current_labels.append(os.path.splitext(os.path.basename(p))[0])
                    except Exception as e:
                        print(f"[VIDEO DEBUG] Failed to load {p}: {e}")
                print(f"[VIDEO DEBUG] Current labels: {current_labels}")

                # 2. Prepare new SAM masks and their labels
                new_labels = []
                new_masks = []
                print(f"[VIDEO DEBUG] self.corrected_label: {self.corrected_label}")
                for mask_idx, mask in enumerate(frame_masks):
                    mask_np = mask.cpu().numpy() if hasattr(mask, 'cpu') else mask
                    mask_bin = (mask_np > 0.05).astype(np.uint8)
                    mask_bin = np.squeeze(mask_bin)
                    if mask_bin.ndim != 2:
                        raise ValueError(f"Mask for contours must be 2D, got shape {mask_bin.shape}")
                    # Edge-based refinement (same as run_sam_on_boxes)
                    ch0_path = self.get_image_path(self.block, self.nanowell, str(target_frame), 'CH0')
                    ch0_image = cv2.imread(ch0_path, cv2.IMREAD_GRAYSCALE)
                    if ch0_image is not None:
                        edges = get_edge_map(ch0_image)
                        mask_bin = refine_mask_with_edges(mask_bin, edges)
                    contours, _ = cv2.findContours(mask_bin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    if contours:
                        largest = max(contours, key=cv2.contourArea)
                        # Apply smoothing (use 8.0 as default, or pass from parent if available)
                        smooth_strength = 8.0
                        try:
                            if hasattr(self, 'parent') and hasattr(self.parent(), 'gaussian_sigma_spinbox'):
                                smooth_strength = self.parent().gaussian_sigma_spinbox.value()
                        except Exception:
                            pass
                        largest = smoothed_contour(largest, smooth_strength)
                        mask_clean = np.zeros_like(mask_bin)
                        cv2.drawContours(mask_clean, [largest], -1, 1, thickness=-1)
                    else:
                        mask_clean = np.zeros_like(mask_bin)

                    # Use the corrected_label if provided, otherwise fallback
                    label = None
                    if self.corrected_label:
                        label = self.corrected_label
                        print(f"[VIDEO DEBUG] Using corrected_label for new mask: {label}")
                    if not label or not str(label).strip():
                        label = f"cell{mask_idx+1}"
                        print(f"[VIDEO DEBUG] Using fallback label for new mask: {label}")
                    print(f"[VIDEO DEBUG] New mask: label={label}, shape={mask_clean.shape}, unique={np.unique(mask_clean)}")
                    new_labels.append(label)
                    new_masks.append(mask_clean)
                print(f"[VIDEO DEBUG] New SAM labels: {new_labels}")

                # 3. Merge: preserve masks for uncorrected cells, replace masks for corrected cells
                merged = {}
                boxes = self.boxes
                
                # Special case: single box correction with last_deleted_label
                if len(boxes) == 1 and hasattr(self, 'last_deleted_label') and self.last_deleted_label:
                    corrected_label = self.last_deleted_label
                    # Preserve all existing masks except the corrected one
                    for label, mask in zip(current_labels, current_masks):
                        if label != corrected_label:
                            merged[label] = mask
                    # Use the deleted label for the new mask
                    merged[corrected_label] = new_masks[0]
                else:
                    # Default: preserve all existing masks for cells that are NOT being corrected
                    for label, mask in zip(current_labels, current_masks):
                        if label not in new_labels:
                            merged[label] = mask
                    # Then, add/replace with new SAM masks for corrected cells
                    # Handle case where multiple boxes might have same label
                    for i, (label, mask) in enumerate(zip(new_labels, new_masks)):
                        if label in merged:
                            # If label already exists, create unique label
                            base_label = label
                            counter = 1
                ...
                print(f"[VIDEO DEBUG] Merging masks: {list(merged.keys())}")

                # 4. Save all merged masks into the SAM mask folder
                for label, mask in merged.items():
                    if not label or not str(label).strip():
                        print(f"[VIDEO DEBUG] Refusing to save mask with empty label")
                        continue
                    print(f"[VIDEO DEBUG] Saving mask {label}: shape={mask.shape}, unique={np.unique(mask)}")
                    np.save(os.path.join(sam_dir, f"{label}.npy"), mask)
                    print(f"[VIDEO DEBUG] Saved SAM mask to {os.path.join(sam_dir, f'{label}.npy')}")

                # Update bounding boxes for next frame using robust tracking
                if len(frame_masks) > 0:
                    updated_boxes = []
                    for box_idx, (box, mask) in enumerate(zip(self.boxes, frame_masks)):
                        # Use robust tracking with motion prediction
                        updated_box = self.update_bbox_from_mask_robust(
                            mask, box, target_frame, self.tracking_histories[box_idx]
                        )
                ...
                self.frame_complete.emit(target_frame)

            # Clear GPU cache every 5 frames to prevent memory buildup
            if torch.cuda.is_available() and (frame_offset + 1) % 5 == 0:
                torch.cuda.empty_cache()
                print(f"Cleared GPU cache after frame {target_frame}")

            # Clear GPU cache after processing
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                print("Cleared GPU cache after video processing")

            self.finished.emit(True, f"Successfully processed {self.frames_to_process} frames!")

        except Exception as e:
            self.finished.emit(False, f"Error during video processing: {str(e)}")
            print(f"Error in VideoPredictionWorker: {e}")
            import traceback
            traceback.print_exc()
...
class ContourZoomWidget(QWidget):
    ...
    def __init__(self, base_path: str, label_text: str, masks=None, mask_type: str = None,flagged_frames: list = None, parent: QWidget = None):
        ...
        self.box_button.setToolTip("Toggle box drawing mode (Shortcut: B)")
        ...
        btn_run_sam.setToolTip("Run Segment-Anything on your boxes (Shortcut: Enter or Ctrl+R)")
        btn_run_sam.clicked.connect(self.run_sam_on_boxes)
        sam_layout.addWidget(btn_run_sam)

        # Add selective contour replacement button
        btn_replace_contour = QPushButton("Replace Selected", self)
        btn_replace_contour.setToolTip("Replace only the selected contour with SAM prediction (keeps other contours)")
        btn_replace_contour.clicked.connect(self.run_sam_replace_selected_contour)
        sam_layout.addWidget(btn_replace_contour)

        # Add new video prediction button
        btn_run_sam_video = QPushButton("Run SAM Video", self)
        btn_run_sam_video.setToolTip("Run SAM prediction for next 10 frames using drawn boxes (Shortcut: V)")
        ...
        # self.save_as_freehand_checkbox.setChecked(False)
        ...
        gaussian_label = QLabel("Smooth ??:", self)
        self.gaussian_sigma_spinbox = QDoubleSpinBox(self)
        self.gaussian_sigma_spinbox.setMinimum(0.1)
        self.gaussian_sigma_spinbox.setMaximum(20.0)
        self.gaussian_sigma_spinbox.setValue(8.0)
        ...
        # Restore the previous '????' icon button at the top-right, slightly higher
        self.info_button = QPushButton("????", self)
        self.info_button.setFixedSize(25, 25)
        self.info_button.setToolTip("Show keyboard shortcuts (Shortcut: I)")
        self.info_button.setStyleSheet("border: none; font-size: 16pt; background: transparent;")
        self.info_button.clicked.connect(self.show_keyboard_shortcuts)
        self.info_button.move(self.width() - 36, 2.5)  # Move higher (y=2)
        ...
        self.edit_mode_button.setToolTip("Toggle nudge/edit mode (Shortcut: N)")
        ...
        self.rubber_band_button.setToolTip("Toggle rubber band mode (Shortcut: U)")
        ...
        self.region_growth_button.setToolTip("Toggle region growth mode (Shortcut: O)")
    ...
    def scaled_points(self):
        # Convert QPoints into resolution list
        W, H = self.ch0_image.shape[1], self.ch0_image.shape[0]
        pw, ph = self.image_label.pixmap().width(), self.image_label.pixmap().height()
        off_x = (self.image_label.width()  - pw) / 2
        off_y = (self.image_label.height() - ph) / 2
        ...
        # Prepare scaled points and corresponding labels
        points = []
        labels = []
        for point, is_foreground in self.image_label.prompts:
            x_coord = point.x()
            y_coord = point.y()
    ...
    def run_sam_on_boxes(self):
        ...
        # Get prompts if used
        points, labels = self.scaled_points()

        # 0) make sure the predictor has the image
        self._samWorker.predictor.set_image(self.fluorescence_overlay)

        # 1) convert to torch boxes - use sam_worker's device instead of global device
        input_boxes = torch.tensor(boxes, device=self._samWorker.device).float()
        ...
        if points and labels:
            points_tensor = torch.tensor(points, dtype=torch.float32, device=self._samWorker.device).float()
            points_trans = self._samWorker.predictor.transform.apply_coords_torch(
                points_tensor.squeeze(0), (H, W)
            ).unsqueeze(0)
            labels_trans = torch.tensor(labels, device=self._samWorker.device).long().unsqueeze(0)

            masks, _, _ = self._samWorker.predictor.predict_torch(
                point_coords=points_trans,
                point_labels=labels_trans,
                boxes=transformed,
        ...
        # Save raw SAM masks to SAM folder
        block, nanowell, frame = self.label_text.split("-")
        ...
        # 1. Load existing masks from the current mask type (e.g., MRCNN)
        current_mask_paths = self.get_mask_paths(block, nanowell, frame_num, self.mask_type)
        current_labels = []
        current_masks = []
        for p in current_mask_paths:
            try:
                current_masks.append(np.load(p))
                current_labels.append(os.path.splitext(os.path.basename(p))[0])
            except Exception:
                pass
        ...
        for mask_idx, mask in enumerate(masks):
            ...
            if contours:
                largest = max(contours, key=cv2.contourArea)
                smooth_strength = self.gaussian_sigma_spinbox.value() if hasattr(self, 'gaussian_sigma_spinbox') else 5
                largest = smoothed_contour(largest, smooth_strength)
                mask_clean = np.zeros_like(mask_bin)
                cv2.drawContours(mask_clean, [largest], -1, 1, thickness=-1)
            else:
                mask_clean = np.zeros_like(mask_bin)
            # Try to get label from tracking_labels if available
            label = None
            if hasattr(self.image_label, 'tracking_labels') and mask_idx < len(self.image_label.tracking_labels):
                label = self.image_label.tracking_labels[mask_idx]
            if not label or not str(label).strip():
                # Fallback to a default label if label is empty or None
                label = f"cell{mask_idx+1}"
            new_labels.append(label)
            new_masks.append(mask_clean)
        ...
        # 3. Merge: preserve masks for uncorrected cells, replace masks for corrected cells
        merged = {}
        print(f"[DEBUG] Merging: {len(current_labels)} current masks, {len(new_labels)} new SAM masks")
        ...
        if len(boxes) == 1 and hasattr(self, 'last_deleted_label') and self.last_deleted_label:
            corrected_label = self.last_deleted_label
            print(f"[DEBUG] Single box correction for label: {corrected_label}")
            # Preserve all existing masks except the corrected one
            for label, mask in zip(current_labels, current_masks):
                if label != corrected_label:
                    merged[label] = mask
                    print(f"[DEBUG] Preserved existing mask: {label}")
            # Use the deleted label for the new mask
            merged[corrected_label] = new_masks[0]
            print(f"[DEBUG] Replaced mask for corrected label: {corrected_label}")
        else:
            # Default: preserve all existing masks for cells that are NOT being corrected
            for label, mask in zip(current_labels, current_masks):
                if label not in new_labels:
                    ...
                    print(f"[DEBUG] Will replace existing mask: {label}")
            ...
            for i, (label, mask) in enumerate(zip(new_labels, new_masks)):
                if label in merged:
                    ...
                    while label in merged:
                        label = f"{base_label}_{counter}"
                        counter += 1
                    print(f"[DEBUG] Created unique label: {label}")
                merged[label] = mask
                print(f"[DEBUG] Added/replaced mask: {label}")

        # 4. Save all merged masks into the SAM mask folder
        print(f"[DEBUG] Saving {len(merged)} masks to {sam_dir}")
        for label, mask in merged.items():
            if not label or not str(label).strip():
                print(f"[ERROR] Refusing to save mask with empty label")
                continue
            print(f"[DEBUG] Saving mask {label}: shape={mask.shape}, unique={np.unique(mask)}")
            np.save(os.path.join(sam_dir, f"{label}.npy"), mask)
            print(f"Saved SAM mask to {os.path.join(sam_dir, f'{label}.npy')}")

        # Get the selected contour index
        selected_idx = self.image_label.selected_contour_index

        # For each predicted mask, extract contour and replace only the selected one
        for mask in masks:
            predicted_mask = mask.cpu().numpy()
            mask_binary = (predicted_mask > 0.05).astype(np.uint8)
            ...
            if cv2.contourArea(largest_contour) < 20:
                continue
            # Convert contour to widget coordinates
            H_img, W_img = self.ch0_image.shape
            ...
            for pt in largest_contour:
                x_img = pt[0][0]
                y_img = pt[0][1]
                x_widget = int(round(off_x + x_img * scale_x))
                y_widget = int(round(off_y + y_img * scale_y))
                new_contour.append(QPoint(x_widget, y_widget))
            # Apply smoothing
            if len(new_contour) > 3:
                interp = self.image_label.interpolate_contour(new_contour, desired_gap=3)
                npts = int(len(interp) * 10 * self.image_label.density_factor)
                smooth_contour = self.image_label.smooth_drawn_contour(interp, s=20, num_points=npts)
            else:
                smooth_contour = new_contour

            # Replace only the selected contour
            if selected_idx != -1 and selected_idx < len(self.image_label.contours):
                self.image_label.contours[selected_idx] = smooth_contour
                self.image_label.raw_contours[selected_idx] = smooth_contour.copy()
            else:
                # If no contour is selected, append as fallback
                self.image_label.contours.append(smooth_contour)
                self.image_label.raw_contours.append(smooth_contour.copy())
        
        
        # Clear drawn boxes and prompts
        self.image_label.sam_boxes.clear()
        self.image_label.prompts.clear()
        self.image_label.is_foreground = True
        if self.box_button.isChecked():
            self.box_button.setChecked(False)
        self.image_label.update()

        # After saving all masks in run_sam_on_boxes
    ...
    def toggle_box_mode(self, enabled):
        ...
        if enabled:
            ...
            if self.region_growth_button.isChecked():
                self.region_growth_button.setChecked(False)
        # When the user toggles off box mode, reset to allow redrawing and segmentation.
        elif not enabled:
            self.image_label.prompts = []
            self.image_label.sam_boxes.clear()
            self.image_label._drawing_box = False
            self.image_label._box_start = QPoint()
            self.image_label._box_current = QPoint()
            self.image_label.is_foreground = True
    ...
    def resizeEvent(self, event):
        ...
        # Move the info button to the top-right on resize
        if hasattr(self, 'info_button'):
            self.info_button.move(self.width() - 36, 2)
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_S:
            ...
            return
        # ????????????????????????????????????????????????????????????
        # Toggle switch between foreground and background
        if key == Qt.Key_P:
            self.image_label.is_foreground = not self.image_label.is_foreground
        ...
        if key == Qt.Key_X:
            if self.image_label.selected_contour_index == -1:
                ...
                if hasattr(self.image_label, 'tracking_labels') and idx < len(self.image_label.tracking_labels):
                    self.last_deleted_label = self.image_label.tracking_labels[idx]
                    print(f"[DEBUG] Deleted contour label stored: {self.last_deleted_label}")
                else:
                    print(f"[DEBUG] No label found for deleted contour at idx {idx}")
        ...
        # R = replace selected contour with SAM prediction
        if key == Qt.Key_R:
            if self.image_label.selected_contour_index == -1:
                print("No contour selected; ignoring R.")
            else:
                self.run_sam_replace_selected_contour()
            event.accept()
            return

        # B = toggle box mode
        if key == Qt.Key_B:
            new_state = not self.box_button.isChecked()
            self.box_button.setChecked(new_state)  # Update UI button state
            self.toggle_box_mode(new_state)
            event.accept()
            return

        # Enter or Ctrl+R = run SAM on boxes (when in box mode or boxes are drawn)
        if key == Qt.Key_Return or key == Qt.Key_Enter or (event.modifiers() & Qt.ControlModifier and key == Qt.Key_R):
            if self.scaled_boxes():  # Check if there are boxes to process
                self.run_sam_on_boxes()
            else:
                print("No boxes drawn; draw boxes first or enable box mode.")
            event.accept()
            return

        # V = run SAM video prediction
        if key == Qt.Key_V:
            self.run_sam_video_prediction()
            event.accept()
            return

        # N = toggle nudge/edit mode
        if key == Qt.Key_N:
            new_state = not self.edit_mode_button.isChecked()
            self.edit_mode_button.setChecked(new_state)  # Update UI button state
            self.toggle_edit_mode(new_state)
            event.accept()
            return

        # U = toggle rubber band mode
        if key == Qt.Key_U:
            new_state = not self.rubber_band_button.isChecked()
            self.rubber_band_button.setChecked(new_state)  # Update UI button state
            self.toggle_rubber_band_mode(new_state)
            event.accept()
            return

        # O = toggle region growth mode (changed from G since G is used for Ground Truth)
        if key == Qt.Key_O:
            new_state = not self.region_growth_button.isChecked()
            self.region_growth_button.setChecked(new_state)  # Update UI button state
            self.toggle_region_growth_mode(new_state)
            event.accept()
            return
    ...
    def switch_mask_type(self, mask_type: str):
        ...
        print(f"[DEBUG] switch_mask_type: mask_type={mask_type}")
        print(f"[DEBUG] Looking in {base_dir}")
        ...
        print(f"[DEBUG] Files in {base_dir}: {files}")
        for fname in files:
            if not fname.endswith(".npy"):
                continue
            # Skip invalid/empty filenames that would create invalid files
            if fname in [".npy", ".npy.npy"] or not fname.replace(".npy", "").strip():
                print(f"[DEBUG] Skipping invalid mask file: {fname}")
                continue
            path = os.path.join(base_dir, fname)
            try:
                arr = np.load(path, allow_pickle=True)
                print(f"[DEBUG] Loading {fname}: shape={getattr(arr, 'shape', None)}, unique={np.unique(arr) if isinstance(arr, np.ndarray) else 'N/A'}")
            except Exception as e:
            ...
            if mask_arr is not None:
                ...
                # Only use the largest contour (if any)
                if cnts:
                    largest = max(cnts, key=cv2.contourArea)
                    if cv2.contourArea(largest) > 5000:
                        pass  # skip if too big (optional)
                    else:
                        pts = [
                            QPoint(
    ...
    def compute_tracking_labels(self):
        ...
        for fname in npy_files:
            # Skip invalid/empty filenames
            if fname in [".npy", ".npy.npy"] or not fname.replace(".npy", "").strip():
                print(f"[DEBUG] Skipping invalid filename in compute_tracking_labels: {fname}")
                continue
            ...
            if m:
                ...
                # Use the filename without extension as the label (e.g., 'cell1')
                label = os.path.splitext(fname)[0]
                # Only add non-empty labels
                if label and label.strip():
                    labels.append(label)
                else:
                    print(f"[DEBUG] Skipping empty label from filename: {fname}")
            ...
            if current_frame >= self.override_start_frame:
                if hasattr(self, "tracking_mapping") and self.tracking_mapping:
                    for idx, new_label in self.tracking_mapping.items():
                        if idx < len(labels) and new_label and str(new_label).strip():
                            labels[idx] = new_label
                        elif idx < len(labels):
                            print(f"[DEBUG] Skipping empty override label for index {idx}")
                    ...
                    for i, lab in enumerate(labels):
                        ...
                        if m:
                            ...
                            if tid in self.tracking_mapping_by_id:
                                new_label = self.tracking_mapping_by_id[tid]
                                if new_label and str(new_label).strip():
                                    labels[i] = new_label
                                else:
                                    print(f"[DEBUG] Skipping empty override label for tracking ID {tid}")
        else:
    ...
    def optimize_gpu_memory(self):
        """Optimize GPU memory usage before heavy operations."""
        if torch.cuda.is_available():
            ...
            for i in range(torch.cuda.device_count()):
                torch.cuda.set_per_process_memory_fraction(0.8, i)  # Use 80% of GPU memory
            
            print("GPU memory optimized for video processing")

    def run_sam_video_prediction(self):
        """
        Run SAM prediction for the next 10 frames using drawn bounding boxes.
        Now runs in background thread to prevent GUI freezing.
        """
        if not self.samLoaded:
            QMessageBox.warning(self, "SAM", "Still loading SAM model ?? please wait.")
            return
        ...
        frames_to_process = min(requested_frames, self.max_frame - current_frame + 1)
        if frames_to_process <= 0:
            QMessageBox.information(self, "SAM Video", "No more frames to process.")
            return

        print(f"Starting video prediction for frames {current_frame} to {current_frame + frames_to_process - 1}")
        ...
        corrected_label = self.last_deleted_label if hasattr(self, 'last_deleted_label') else None
        print(f"[DEBUG] Passing corrected_label to VideoPredictionWorker: {corrected_label}")
        self.video_worker = VideoPredictionWorker(
            sam_worker=self._samWorker,
            ...
            custom_checkpoint=custom_sam_checkpoint,
        ...
        self.video_worker.finished.connect(self.on_video_prediction_finished)
        self.video_worker.frame_complete.connect(self.on_frame_complete)

        # Show progress dialog
        self.video_progress = QProgressDialog("Processing video frames...", "Cancel", 0, frames_to_process, self)
        self.video_progress.setWindowModality(Qt.WindowModal)
        self.video_progress.setAutoClose(True)
        self.video_progress.canceled.connect(self.cancel_video_prediction)
        self.video_progress.show()

        # Start the background processing
        self.video_thread.start()

    def update_video_progress(self, frame_offset, status_message):
        """Update progress dialog with current status."""
        if hasattr(self, 'video_progress') and not self.video_progress.wasCanceled():
            self.video_progress.setValue(frame_offset)
            self.video_progress.setLabelText(status_message)

    def on_frame_complete(self, frame_number):
        """Called when a frame is completed."""
        print(f"Completed frame {frame_number}")

    def cancel_video_prediction(self):
        """Cancel the video prediction process."""
        if hasattr(self, 'video_thread') and self.video_thread.isRunning():
            self.video_thread.quit()
            self.video_thread.wait()
            QMessageBox.information(self, "SAM Video", "Video processing was canceled.")

    def on_video_prediction_finished(self, success, message):
        """Called when video prediction is finished."""
        if hasattr(self, 'video_progress'):
            self.video_progress.close()
            
        if success:
            QMessageBox.information(self, "SAM Video", message)
        else:
            QMessageBox.critical(self, "SAM Video Error", message)
        ...
        # Preserve current mask type and reload contours
        current_mask_type ="SAM" or self.mask_type  
        self.switch_mask_type(current_mask_type)
        print(f"Video prediction complete. Staying on {current_mask_type} mask type.")
    ...
    def expand_search_area_for_sudden_movement(self, original_bbox, movement_threshold=30):
        """
        Expand the search area when sudden movement is detected, but more conservatively.
        This helps catch sudden movements without capturing neighboring cells.
        """
        x1, y1, x2, y2 = original_bbox
        w, h = x2 - x1, y2 - y1
        
        # IMPROVED: More conservative expansion to avoid capturing neighbors
        # Only expand by 25% instead of 50% to keep boxes focused
        expansion_x = int(w * 0.25)  # Reduced from 0.5
        expansion_y = int(h * 0.25)  # Reduced from 0.5
        
        # IMPROVED: Cap the maximum expansion to prevent oversized boxes
        max_expansion = 20  # Maximum pixels to expand in any direction
        expansion_x = min(expansion_x, max_expansion)
        expansion_y = min(expansion_y, max_expansion)
        ...
        # IMPROVED: Ensure expanded box doesn't get too large
        max_size = 100
        expanded_w = expanded_x2 - expanded_x1
        expanded_h = expanded_y2 - expanded_y1
        
        if expanded_w > max_size:
            # Center the box and cap the width
            center_x = (expanded_x1 + expanded_x2) / 2
            expanded_x1 = int(center_x - max_size / 2)
            expanded_x2 = expanded_x1 + max_size
        
        if expanded_h > max_size:
            # Center the box and cap the height
            center_y = (expanded_y1 + expanded_y2) / 2
            expanded_y1 = int(center_y - max_size / 2)
            expanded_y2 = expanded_y1 + max_size
    ...
    def save_contours_as_freehand(self, contours, block, nanowell, frame, mask_idx):
        ...
        # Save raw coordinate arrays (same as save_contours function)
        for i, pts in enumerate(raw_contours):
            # Save the raw points array with tracking label naming
            # Use a default label since we don't have tracking labels in video prediction
            fname = f"cell{mask_idx}_{i+1}.npy"
            raw_path = os.path.join(freehand_dir, fname)
            np.save(raw_path, np.array(pts, dtype=object), allow_pickle=True)
    ...
    def get_secondary_gpu_device(self):
        """Get the secondary GPU device for other operations."""
        if torch.cuda.device_count() > 1:
            return "cuda:1"  # Use the second GPU
        return "cpu"
    ...
    def run_sam_replace_selected_contour(self):
        """
        Replace only the selected contour with SAM prediction while keeping all other contours intact.
        This allows selective improvement of individual cell contours.
        """
        if not self.samLoaded:
            QMessageBox.warning(self, "SAM", "Still loading SAM model ?? please wait.")
            return

        # Check if a contour is selected
        selected_idx = self.image_label.selected_contour_index
        if selected_idx == -1:
            QMessageBox.warning(self, "SAM", "Please select a contour to replace first.")
            return

        # Check if a bounding box is drawn
        boxes = self.scaled_boxes()
        if not boxes:
            QMessageBox.warning(self, "SAM", "Draw a bounding box around the cell you want to replace.")
            return

        if len(boxes) > 1:
            QMessageBox.warning(self, "SAM", "Please draw only one bounding box for the selected contour.")
            return

        # Get prompts if used
        points, labels = self.scaled_points()

        # Make sure the predictor has the image
        self._samWorker.predictor.set_image(self.fluorescence_overlay)

        # Convert to torch box
        input_boxes = torch.tensor(boxes, device=self._samWorker.device).float()
        H, W = self.fluorescence_overlay.shape[:2]
        transformed = self._samWorker.predictor.transform.apply_boxes_torch(input_boxes, (H, W))
        ...
        try:
            if points and labels:
                # Convert points to torch coordinates
                points_tensor = torch.tensor(points, dtype=torch.float32, device=self._samWorker.device).float()
                points_trans = self._samWorker.predictor.transform.apply_coords_torch(
                    points_tensor.squeeze(0), (H, W)
                ).unsqueeze(0)
            ...
            mask_binary = np.squeeze(mask_binary)
            if mask_binary.ndim != 2:
                raise ValueError(f"Mask for contours must be 2D, got shape {mask_binary.shape}")
            mask_binary = mask_binary.astype(np.uint8)
            contours, _ = cv2.findContours(mask_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            
            if not contours:
                QMessageBox.warning(self, "SAM", "No contour detected in the predicted mask.")
                return

            # Get the largest contour from the prediction
            largest_contour = max(contours, key=cv2.contourArea)
            
            if cv2.contourArea(largest_contour) < 20:
                QMessageBox.warning(self, "SAM", "Predicted contour is too small.")
                return
            ...
            # Convert contour points to widget coordinates
            new_contour = []
            for pt in largest_contour:
                x_img = pt[0][0]
                y_img = pt[0][1]
            ...
            # Apply smoothing to the new contour
            if len(new_contour) > 3:
                interp = self.image_label.interpolate_contour(new_contour, desired_gap=3)
                npts = int(len(interp) * 10 * self.image_label.density_factor)
                smooth_contour = self.image_label.smooth_drawn_contour(interp, s=20, num_points=npts)
            ...
            # Replace the selected contour with the new one
            if selected_idx < len(self.image_label.contours):
                self.image_label.contours[selected_idx] = smooth_contour
                self.image_label.raw_contours[selected_idx] = smooth_contour.copy()
                ...
                QMessageBox.information(self, "SAM", f"Successfully replaced contour {selected_idx + 1} with SAM prediction.")
            else:
                QMessageBox.warning(self, "SAM", "Selected contour index is out of range.")

        except Exception as e:
            QMessageBox.critical(self, "SAM Error", f"Error during SAM prediction: {str(e)}")
            print(f"Error in run_sam_replace_selected_contour: {e}")
            import traceback
            traceback.print_exc()
    ...
    def show_keyboard_shortcuts(self):
        ...
        <b>SAM Tools:</b>
        <ul>
            <li><b>B</b> - Toggle Box Mode</li>
            <li><b>Enter</b> or <b>Ctrl+R</b> - Run SAM on boxes</li>
            <li><b>V</b> - Run SAM Video Prediction</li>
        </ul>
        <b>Edit Modes:</b>
        <ul>
            <li><b>N</b> - Toggle Nudge/Edit Mode</li>
            <li><b>U</b> - Toggle Rubber Band Mode</li>
            <li><b>O</b> - Toggle Region Growth Mode</li>
        </ul>
        <b>Navigation:</b>
        <ul>
            <li><b>?? ??</b> - Previous/Next Frame</li>
            <li><b>?? ??</b> - Previous/Next Flagged Frame</li>
            <li><b>0</b> - Reset to Original Image</li>
        </ul>
        <b>Actions:</b>
        <ul>
            <li><b>S</b> - Save Contours</li>
            <li><b>X</b> - Delete Selected Contour</li>
            <li><b>C</b> - Fix Cell Type</li>
            <li><b>F</b> - Swap Selected Contours (multi-select)</li>
            <li><b>Ctrl+I</b> - Toggle Multi-Select Mode</li>
        </ul>
        <b>Mask Types:</b>
        <ul>
            <li><b>1-6</b> - Switch between mask types (MRCNN, FRCNN, SAM, FreeHand, GT, CT, Track)</li>
            <li><b>G</b> - Ground Truth</li>
        </ul>
        <b>Tips:</b>
        <ul>
            <li>Hover over buttons to see their shortcuts</li>
            <li>Press the same key twice to toggle off modes</li>
            <li>Use Enter after drawing boxes for quick SAM prediction</li>
            <li>Press <b>I</b> to open this help dialog</li>
        </ul>
        """
...
class ContourDrawingLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if self.box_mode and event.button() == Qt.RightButton:
            point = event.pos()
            self.prompts.append((point, int(self.is_foreground)))
            self.update()
            return
        ...
        try:
            ...
            if self.show_tracking_labels and self.tracking_labels:
                ...
                for i, contour in enumerate(self.contours):
                    ...
                    # Only show label if it looks like a tracking label (e.g., A12, B5, etc.) and does not start with 'cell'
                    if not re.match(r"^[A-Za-z]+\d+$", label) or label.lower().startswith('cell'):
                        continue  # skip fallback or filename-based labels
            ...
            if self.box_mode:
                ...
                # Represent foreground with green dots and background with red dots
                if self.prompts:
                    for point, is_foreground in self.prompts:
                        if is_foreground == 0:
